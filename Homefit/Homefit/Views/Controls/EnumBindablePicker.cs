...
namespace Homefit.Views.Controls
{
    public class EnumBindablePicker<T>: Picker where T : struct
    {
        public EnumBindablePicker()
        {
            ...
            foreach (var value in Objectif.GetValues(typeof(T)))
            {
        ...
        private void OnSelectedIndexChanged(object sender, EventArgs eventArgs)
        {
            ...
            else
            {
                ...
                T match;
                if (!Objectif.TryParse<T>(Items[SelectedIndex], out match))
                {
                    //find enum by Description
                    match = GetEnumByDescription(Items[SelectedIndex]);
                }
                SelectedItem = (T)Objectif.Parse(typeof(T), match.ToString());
            }
        }
        ...
        {
            return Objectif.GetValues(typeof(T)).Cast<T>().FirstOrDefault(x => string.Equals(GetEnumDescription(x), description));
        }
    }
}
